import math
import random
from exp2bs.models.networks_face2face import BaseNetwork
import torch
...
class EqualConv2d(nn.Module):
    ...
    def __repr__(self):
        ...
        # )
        return '{}({}, {}, {}, stride={}, padding={})'.format(self.__class__.__name__, self.weight.shape[1],
                                                              self.weight.shape[0], self.weight.shape[2],
...
class ModulatedConv2d(nn.Module):
    ...
    def __repr__(self):
        ...
        # )
        return '{}({}, {}, {}, upsample={}, downsample={})'.format(self.__class__.__name__, self.in_channel,
                                                                   self.out_channel, self.kernel_size,
                                                                   self.upsample, self.downsample)

    def forward(self, input, style):
        batch, in_channel, height, width = input.shape                      # (N, 512, 4, 4)

        style = self.modulation(style).view(batch, 1, in_channel, 1, 1)     # (N, 1, in_c, 1, 1)
        weight = self.scale * self.weight * style  # (1, out_c, in_c, k, k)*(N, 1, in_c, 1, 1) = (N, out_c, in_c, k, k)

        if self.demodulate:
            demod = torch.rsqrt(weight.pow(2).sum([2, 3, 4]) + 1e-8)            # (N, out_c)
            weight = weight * demod.view(batch, self.out_channel, 1, 1, 1)      # (N, out_c, in_c, k, k)

        weight = weight.view(batch * self.out_channel, in_channel, self.kernel_size, self.kernel_size)      # (N*out_c, in_c, k, k)
...
class StyledConv(nn.Module):
    ...
    def forward(self, input, style, noise=None):
        out, _ = self.conv(input, style)        # same shape as input
        out = self.noise(out, noise=noise)      # same shape as input
        # out = out + self.bias
        out = self.activate(out)                # same shape as input
...
class StyleGAN2Generator(BaseNetwork):
    ...
    def forward(
        ...
        if len(styles) < 2:
            ...
            latent = torch.cat([latent, latent2], 1)        # (N, num_layers, style_dim)

        if identity_style is not None:
            out = identity_style
        else:
            out = self.input(latent)            # (N, 512, 4, 4)
        out = self.conv1(out, latent[:, 0], noise=noise[0])     # (N, 512, 4, 4)

        skip, style_rgb = self.to_rgb1(out, latent[:, 1])       # (N, 3, 4, 4), (N, 1, 512, 1, 1)
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            out = conv1(out, latent[:, i], noise=noise1)            # (N, 512, 8, 8)
            out = conv2(out, latent[:, i + 1], noise=noise2)        # (N, 512, 8, 8)
