...
class BSRegressModel(BaseModel):
    # Blendshape Regression Model
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_img', 'fake_img', 'person_img', 'virtual_img']
        self.model_names = ['V']
        ...
        if self.isTrain:
            ...
            self.criterionRegress = torch.nn.MSELoss().to(self.device)
    ...
    def set_input(self, input):
        ...
        self.person_img = input['images_real'].to(self.device)
        self.blendshape_tgt = input['blendshapes'].to(self.device)
        self.image_paths = input['img_paths']

    def forward(self):
        self.blendshape_pred = self.netV.forward_feature(self.real_img)
        with torch.no_grad():
            style = self.netE(self.blendshape_pred)
            self.fake_img, _ = self.netG([style])

    def forward_visuals(self):
        with torch.no_grad():
            blendshape_pred = self.netV.forward_feature(self.real_img)
            style = self.netE(blendshape_pred)
            self.fake_img, _ = self.netG([style])

            blendshape_person = self.netV.forward_feature(self.person_img)
            style = self.netE(blendshape_person)
            self.virtual_img, _ = self.netG([style])
    ...
    def backward_G(self):
        self.loss_L2 = self.criterionRegress(self.blendshape_tgt, self.blendshape_pred)
