...
from landmark2exp.util import util
# from util.nvdiffrast import MeshRenderer
from landmark2exp.util.preprocess import estimate_norm_torch
import json
...
class FaceReconModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--bfm_folder', type=str, default='landmark2exp/BFM')
    ...
    def compute_visuals(self):
        with torch.no_grad():
            ...
            if self.gt_lm is not None:
                ...
                # save pred_lm_numpy as landmark_results
                reshaped_data = pred_lm_numpy.reshape(-1, 2)
                df = pd.DataFrame(reshaped_data, columns=['X', 'Y'])
                df.to_csv("landmark_results/%s.csv" % self.image_name, index=False)

                # 01/13
                # output_vis_numpy = util.draw_landmarks(output_vis_numpy_raw, gt_lm_numpy, 'b')
                # output_vis_numpy = output_vis_numpy_raw
                output_vis_numpy = np.full((1, 224, 224, 3), 255.0, dtype=float)

                output_vis_numpy = util.draw_landmarks(output_vis_numpy, pred_lm_numpy, 'r')

                output_vis_numpy = np.concatenate((input_img_numpy,
                                                   output_vis_numpy_raw, output_vis_numpy), axis=-2)
                # output_vis_numpy = np.concatenate((output_vis_numpy_raw, output_vis_numpy), axis=-2)
            else:
                output_vis_numpy = np.concatenate((input_img_numpy,
