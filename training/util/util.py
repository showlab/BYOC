...
def tensor2im_v2(image_tensor, imtype=np.uint8, normalize=True, tile=True):
...
def save_image_v2(image_numpy, image_path, create_dir=False):
    if create_dir:
...
def save_torch_img(img, save_path):
    image_numpy = tensor2im_v2(img, tile=False)
    save_image_v2(image_numpy, save_path, create_dir=True)
    return image_numpy
...
def find_class_in_module(target_cls_name, module):
    # CP:target_cls_name == "exp2bsleakyclampencoder"
    # CP:module == "models.networks_face2face.encoder"
