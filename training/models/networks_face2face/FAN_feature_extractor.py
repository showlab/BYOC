...
class FAN_use(nn.Module):
    def __init__(self, opt):
        ...
        if self.opt.crop_size == 224:
            self.fc = nn.Linear(28 * 28, self.opt.motion_dim)
        elif self.opt.crop_size == 256:
            self.fc = nn.Linear(32 * 32, self.opt.motion_dim)
        elif self.opt.crop_size == 112:
            self.fc = nn.Linear(14 * 14, self.opt.motion_dim)
